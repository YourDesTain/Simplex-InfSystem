function afficher(variables,contraintes)
{
    ...
    while(j < variables)
    {
      ...
      try {
        ...
        if (e instanceof ReferenceError)
        {
          ...
          t.setAttribute('placeholder','пустое значение');
          t = 1;
        }
      }
      if(t.value == "")
  ...
  while(i < variables)
  {
    ...
    try {
      ...
      if (e instanceof ReferenceError)
      {
        ...
        t1.setAttribute('placeholder','пустое значение');
        t = 1;
      }
    }
    if(t1.value == "")
  ...
  if(a.checked)
  {
    ...
    if(b.includes('a') == 1)
    {
        ...
        {
            if(a == 0)
              document.getElementById('racine').appendChild(document.createTextNode('Решение задачи не существует'));
            if(a == -1)
            {
                document.getElementById('racine').appendChild(document.createTextNode('Нерешаемая проблема'));
                non_bor = 1;
            }
        }
        else
